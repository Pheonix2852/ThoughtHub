...
export const BLOGS_QUERY = defineQuery(`*[_type == "startup" && defined(slug.current) && !defined($search) || title match $search || category match $search || author->name match $search] | order(_createdAt desc) {
  _id, 
  title, 
  slug,
  _createdAt,
...
export const BLOG_BY_ID_QUERY = defineQuery(`*[_type == "startup" && _id==$id][0] {
  _id, 
  title, 
  slug,
  _createdAt,
...
export const BLOG_VIEWS_QUERY = defineQuery(`*[_type == "startup" && _id == $id][0]{
   _id, views
  }`)
